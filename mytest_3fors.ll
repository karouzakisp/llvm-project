...
for.body:                                         ; preds = %entry, %for.inc26
  %indvars.iv52 = phi i32 [ 1, %entry ], [ %indvars.iv.next53, %for.inc26 ]
  %sum.048 = phi i32 [ 0, %entry ], [ %sum.4, %for.inc26 ]
  switch i32 %indvars.iv52, label %vector.ph [
    i32 101, label %for.inc26
    i32 1005, label %for.inc26
  ]

vector.ph:                                        ; preds = %for.body, %middle.block
  %sum.144 = phi i32 [ %12, %middle.block ], [ %sum.048, %for.body ]
  %j.143 = phi i16 [ %inc24, %middle.block ], [ 0, %for.body ]
  %0 = insertelement <4 x i32> <i32 poison, i32 0, i32 0, i32 0>, i32 %sum.144, i64 0
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi57 = phi <4 x i32> [ zeroinitializer, %vector.ph ], [ %10, %vector.body ]
...
middle.block:                                     ; preds = %vector.body
  ...
  %inc24 = add nuw nsw i16 %j.143, 1
  %exitcond51.not = icmp eq i16 %inc24, 100
  br i1 %exitcond51.not, label %for.inc26, label %vector.ph, !llvm.loop !9

for.inc26:                                        ; preds = %middle.block, %for.body, %for.body
  %sum.4 = phi i32 [ %sum.048, %for.body ], [ %sum.048, %for.body ], [ %12, %middle.block ]
  %indvars.iv.next53 = add nuw nsw i32 %indvars.iv52, 1
  %exitcond55.not = icmp eq i32 %indvars.iv.next53, 50
  br i1 %exitcond55.not, label %for.end28, label %for.body, !llvm.loop !10
...
!4 = !{!"clang version 16.0.6 (git@github.com:karouzakisp/llvm-project.git c86e0190649361a4ba9781d6d1ef23712a03bce8)"}
!5 = distinct !{!5, !6, !7, !8}
