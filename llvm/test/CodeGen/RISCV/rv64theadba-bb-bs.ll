...
; RUN: llc -march=riscv64 -mattr=+xtheadba -mattr =+xtheadbb -mattr=+xtheadbs < %s | FileCheck %s -verify-machineinstr < %s
...
; CHECK-NEXT:    lui a1, %hi({{\.?LCPI[0-9]+_[0-9]+}})
; CHECK-NEXT:    ld a1, %lo({{\.?LCPI[0-9]+_[0-9]+}})(a1)
; CHECK-NEXT:    lui a2, %hi({{\.?LCPI[0-9]+_[0-9]+}})
; CHECK-NEXT:    ld a2, %lo({{\.?LCPI[0-9]+_[0-9]+}})(a2)
