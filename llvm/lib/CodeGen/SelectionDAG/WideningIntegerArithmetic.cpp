...
void WideningIntegerArithmetic::solve(CodeGenOpt::Level OL){
  ...
  for (SDNode &Node : DAG.allnodes()){
    if(!IsSolved(&Node)  && isInteger(&Node) ){
      std::cout << "Visiting NodeOPC " << Node.getOpcode() << OpcodesToStr[Node.getOpcode()] << '\n'; 
      visit_widening(&Node);  
    }
    
  }
  std::cout << "Number of Truncates : " << TruncCounter << '\n';
  std::cout << "Number of Extensions : " << ExtCounter << '\n';
} 
...
WideningIntegerArithmetic::closure(SDNode *Node){
  ...
  do{
    Changed = false;
    auto FillsList = visitFILL(Node);
    auto WidensList = visitWIDEN(Node);
    auto GarbageWidenList = visitWIDEN_GARBAGE(Node); 
    auto NarrowList = visitNARROW(Node);
